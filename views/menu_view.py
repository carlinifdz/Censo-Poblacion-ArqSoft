...
class MenuView(ttk.Frame):
    def __init__(self, master, username, go_domicilios, go_habitantes, go_reportes, go_registro,do_logout, do_exit):
        ...
        ttk.Button(self, text="Registro de habitantes", command=self.go_registro)\
            .grid(row=2, column=0, sticky="ew", padx=6, pady=6, columnspan=2)
        ...
        ttk.Button(self, text="Cerrar sesión", command=self.do_logout)\
            .grid(row=4, column=0, sticky="ew", padx=6, pady=6)
